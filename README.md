...
> **📢 最新更新 (2025-08-14) - v3.0.0 數據匯入系統重大更新**  
> - 🆕 **結構化目錄系統**: 支援 quarterly/daily/metadata 分類架構
> - 🆕 **精確匯入控制**: 按類別、市場、類型精確匯入數據
> - 🆕 **完整工作流程**: 爬取→匯入的一體化解決方案
> - ✅ 新增 v3.0 import-fundamental-api.ts (--category, --market, --type)
> - ✅ 重寫 package.json 匯入命令，支援組合操作
> - ✅ 保持對舊格式的向後兼容性
> - 📦 支援 npm run import:fundamental:tw:quarterly 等組合命令
...
```bash
# 1️⃣ 爬取財務數據到結構化目錄
npm run crawl:tw:quarterly          # 爬取台灣季度財務數據

# 2️⃣ 匯入數據到後端 API
npm run import:fundamental:tw:quarterly  # 匯入台灣季度數據

# 🎯 或者快速設置 (跳過爬蟲，使用現有數據)
npm run setup:structured            # 使用 output/ 現有數據
```
...
| 命令類型 | 範例命令 | 說明 |
|---------|---------|------|
| **按類別匯入** | `npm run import:fundamental:quarterly` | 所有市場的季度數據 |
| **按市場匯入** | `npm run import:fundamental:tw` | 台灣所有類型數據 |
| **組合匯入** | `npm run import:fundamental:tw:quarterly` | 台灣季度數據 |
| **類型匯入** | `npm run import:balance-sheet` | 所有資產負債表 |
| **批量匯入** | `npm run import:fundamental:batch` | 匯入所有類別 |
...
- **[CLAUDE.md](./CLAUDE.md)** - Claude 開發協作指南和六大核心原則
- **[package.json](./package.json)** - 完整命令列表

### 歸檔文檔

- **[完整工作流程歸檔](docs/20250814-complete-workflow-archive.md)** - 詳細工作流程參考 (歸檔)
