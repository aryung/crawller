 ...
 * 轉換爬蟲數據為 API 格式
 */
function convertToApiFormat(record: any): any {
  // 🔍 診斷模式：檢查是否包含禁用欄位
  console.log(`\n🔍 [診斷] 開始處理: ${record.symbolCode} (${record.exchangeArea})`);
  console.log(`[診斷] 數據來源: ${record.dataSource || 'unknown'}`);
  
  // 檢查台灣數據是否意外包含美國專用欄位
  if (record.exchangeArea === 'TPE' || record.exchangeArea === 'TW') {
    const forbiddenUSFields = [
      'totalCapitalization', 'netTangibleAssets', 'commonStockEquity', 'netDebt',
      'basicAverageShares', 'dilutedAverageShares', 'pretaxIncome', 'taxProvision',
    ...
    for (const field of forbiddenUSFields) {
      if (record[field] !== undefined) {
        console.error(`❌ [嚴重錯誤] 台灣數據 ${record.symbolCode} 包含禁用的美國欄位: ${field}=${record[field]}`);
        hasError = true;
      }
    }
  ...
  if (record.exchangeArea === 'TPE' || record.exchangeArea === 'TW') {
    ...
    console.log(`[SymbolCode] 台灣股票清理: ${record.symbolCode} → ${cleanSymbolCode}`);
  }
  ...
  if (record.exchangeArea === 'TW' || record.exchangeArea === 'TPE') {
  ...
  const USSpecificFields = new Set([
    'netTangibleAssets', 'totalCapitalization', 'commonStockEquity', 'netDebt',
    'basicAverageShares', 'dilutedAverageShares', 'pretaxIncome', 'taxProvision',
    'interestIncome', 'interestExpense'
  ]);
  
  const TWSpecificFields = new Set([
    'monthlyRevenue', 'yoyGrowth', 'momGrowth'
  ]);
  
  const JPSpecificFields = new Set([
    'operatingProfit', 'ordinaryProfit'
  ]);
  
  // 添加其他未映射的欄位到 regionalData，但防止跨地區欄位污染
  const standardFields = new Set([
    'symbolCode', 'exchangeArea', 'reportDate', 'fiscalYear', 'fiscalMonth', 
    'reportType', 'dataSource', 'lastUpdated', 'revenue', 'costOfGoodsSold',
    'grossProfit', 'operatingExpenses', 'operatingIncome', 'netIncome', 'ebitda',
    'eps', 'dilutedEPS', 'totalAssets', 'currentAssets', 'totalLiabilities',
    'currentLiabilities', 'shareholdersEquity', 'totalDebt', 'longTermDebt',
    'shortTermDebt', 'cashAndEquivalents', 'bookValuePerShare', 'sharesOutstanding',
    'operatingCashFlow', 'investingCashFlow', 'financingCashFlow', 'freeCashFlow',
    'capex', 'debtIssuance', 'debtRepayment', 'dividendPayments', 'regionalData',
    ...USSpecificFields, ...TWSpecificFields, ...JPSpecificFields
  ]);
  
  for (const key in record) {
    if (!standardFields.has(key) && record[key] !== undefined && record[key] !== null) {
      // 跨地區欄位污染檢查
      if (record.exchangeArea === 'TPE' || record.exchangeArea === 'TW') {
        // 台灣數據不應包含美國或日本特有欄位
        if (USSpecificFields.has(key) || JPSpecificFields.has(key)) {
          console.warn(`[轉換警告] 台灣數據 ${cleanSymbolCode} 包含跨地區欄位: ${key}=${record[key]}，已忽略`);
          continue;
        }
      } else if (record.exchangeArea === 'US') {
        // 美國數據不應包含台灣或日本特有欄位
        if (TWSpecificFields.has(key) || JPSpecificFields.has(key)) {
          console.warn(`[轉換警告] 美國數據 ${cleanSymbolCode} 包含跨地區欄位: ${key}=${record[key]}，已忽略`);
          continue;
        }
      } else if (record.exchangeArea === 'JP') {
        // 日本數據不應包含台灣或美國特有欄位
        if (TWSpecificFields.has(key) || USSpecificFields.has(key)) {
          console.warn(`[轉換警告] 日本數據 ${cleanSymbolCode} 包含跨地區欄位: ${key}=${record[key]}，已忽略`);
          continue;
        }
      }
...
function validateRecord(record: any): boolean {
  ...
  // 交易所區域驗證
  if (!['TPE', 'TW', 'US', 'JP'].includes(record.exchangeArea)) {
    return false;
  }
  ...
  if (isNaN(fiscalYear) || fiscalYear < 1990 || fiscalYear > new Date().getFullYear() + 1) {
    return false;
  }
...
async function importJsonFile(
  ...
  try {
    ...
    for (const [index, crawlResult] of jsonData.results.entries()) {
      ...
      if (!crawlResult.data || !crawlResult.data.data || !Array.isArray(crawlResult.data.data)) {
      ...
      console.log(`📊 [診斷] 結果 ${index + 1}: 包含 ${crawlResult.data.data.length} 條數據記錄`);
      
      for (const [recordIndex, record] of crawlResult.data.data.entries()) {
        console.log(`📊 [診斷] 處理記錄 ${recordIndex + 1}: ${record.symbolCode} (${record.exchangeArea})`);
        ...
        if (validateRecord(record)) {
          ...
          console.warn(`⚠️ [診斷] 記錄 ${recordIndex + 1} 驗證失敗:`, record.symbolCode);
          invalidRecords.push(record);
        }
      }
      
      if (verbose || invalidRecords.length > 0) {
        console.log(`   📋 有效記錄: ${validRecords.length}, 無效記錄: ${invalidRecords.length}`);
      }
      ...
      if (isDryRun) {
        ...
        if (verbose && validRecords.length > 0) {
          console.log(`   範例記錄: ${validRecords[0].symbolCode} (${validRecords[0].exchangeArea}) - ${validRecords[0].reportDate}`);
        }
        ...
        try {
          ...
          const errorMsg = error.response 
            ? `API 錯誤: ${JSON.stringify(error.response.data)}`
            : `網路錯誤: ${error.message}`;
...
async function main() {
  ...
  if (!token) {
    console.warn('⚠️ 未提供 API Token，請確保後端允許無認證存取或使用 --token 參數');
    console.log('   或設定環境變數 BACKEND_API_TOKEN');
  }
    ...
    filesToImport = await new Promise<string[]>((resolve, reject) => {
      glob.glob(globPattern, (error, matches) => {
        if (error) reject(error);
          ...
          // 過濾掉目錄，只保留 .json 檔案
          const jsonFiles = (matches || []).filter(file => {
            try {
              const stat = fs.statSync(file);
              return stat.isFile() && file.endsWith('.json');
            } catch (err) {
  ...
  for (const file of filesToImport) {
    ...
    if (isVerbose || result.errors.length > 0) {
      console.log(`📊 檔案總結: 匯入 ${result.imported}, 失敗 ${result.failed}, 錯誤 ${result.errors.length}`);
    }
    
    console.log(`📈 整體進度: ${processedFiles}/${filesToImport.length} 檔案處理完成`);
  }
  ...
  if (allErrors.length > 0) {
    ...
    if (isVerbose) {
      ...
      console.log(`  ... 還有 ${allErrors.length - 5} 個錯誤 (使用 --verbose 查看全部)`);
    }
  }
