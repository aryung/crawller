...
async function main() {
  ...
  // 找出所有 Yahoo Finance TW Cash Flow Statement 配置文件
  let configFiles = [];
  ...
  if (fs.existsSync(activeDir)) {
    try {
      configFiles = fs.readdirSync(activeDir)
        .filter(file => file.startsWith('yahoo-finance-tw-cash-flow-statement-') && file.endsWith('.json'))
  ...
  if (configFiles.length === 0) {
    console.log('❌ 沒有找到 Yahoo Finance TW Cash Flow Statement 配置文件');
    console.log('💡 請先執行: node scripts/generate-yahoo-tw-configs.js --type=cash-flow-statement');
    process.exit(1);
  }
  ...
  console.log('🔍 Yahoo Finance Taiwan Cash Flow Statement 批量爬蟲');
  console.log('===============================================');
  console.log(`📋 找到 ${configFiles.length} 個現金流量表配置文件`);
  ...
  for (let i = 0; i < configFiles.length; i++) {
    ...
    console.log(`\n[${i + 1}/${configFiles.length}] 處理現金流量表配置: ${configName}`);
  ...
  console.log('\n💡 提示:');
  console.log('   - 檢查 output/ 目錄查看現金流量表爬取結果');
  console.log('   - 使用 --limit=N 參數限制執行數量');
  console.log('   - 失敗的配置可以單獨重新執行');
  console.log('   - 現金流量表數據格式: 「營業現金流、投資現金流、融資現金流」季度/年度表格');
  console.log('   - 輸出格式: fiscalPeriod, operatingCashFlow, investingCashFlow, financingCashFlow, freeCashFlow');
}
