...
async function main() {
  ...
  // 找出所有 Yahoo Finance TW EPS 配置文件
  let configFiles = [];
  ...
  if (fs.existsSync(activeDir)) {
    try {
      configFiles = fs.readdirSync(activeDir)
        .filter(file => file.startsWith('yahoo-finance-tw-eps-') && file.endsWith('.json'))
  ...
  if (configFiles.length === 0) {
    console.log('❌ 沒有找到 Yahoo Finance TW EPS 配置文件');
    console.log('💡 請先執行: node scripts/generate-yahoo-tw-configs.js --type=eps');
    process.exit(1);
  }
  ...
  console.log('🔍 Yahoo Finance Taiwan EPS 批量爬蟲');
  console.log('===============================================');
  console.log(`📋 找到 ${configFiles.length} 個EPS配置文件`);
  ...
  for (let i = 0; i < configFiles.length; i++) {
    ...
    console.log(`\n[${i + 1}/${configFiles.length}] 處理EPS配置: ${configName}`);
  ...
  console.log('\n💡 提示:');
  console.log('   - 檢查 output/ 目錄查看EPS爬取結果');
  console.log('   - 使用 --limit=N 參數限制執行數量');
  console.log('   - 失敗的配置可以單獨重新執行');
  console.log('   - EPS數據格式: 「每股盈餘」季度表格');
  console.log('   - 輸出格式: fiscalPeriod (YYYY-Q1), eps, quarterlyGrowth, yearOverYearGrowth, averagePrice');
}
