...
console.log('🔍 Yahoo Finance Taiwan 配置生成器');
console.log('====================================');

if (templateFiles.length === 0) {
  console.log('❌ 沒有找到 Yahoo Finance Taiwan 模板文件');
  process.exit(1);
}
...
if (!fs.existsSync(stockCodesPath)) {
  console.log('❌ 找不到台灣股票代碼數據文件:', stockCodesPath);
  process.exit(1);
}
  ...
  for (const dir of searchDirs) {
    ...
    if (fs.existsSync(dirPath)) {
      const files = getAllJsonFiles(dirPath)
        .filter(file => file.includes('yahoo-finance-tw-'))
