...
async function main() {
  console.log('🎯 Yahoo Finance US Financials 批處理爬蟲');
  console.log('=============================================');
  
  // 尋找所有 Yahoo Finance US Financials 配置文件
  const configsDir = path.join(__dirname, '../config');
  const configFiles = fs.readdirSync(configsDir)
    .filter(file => file.startsWith('yahoo-finance-us-financials-') && file.endsWith('.json'))
    .map(file => file.replace('.json', ''));
  
  if (configFiles.length === 0) {
    console.log('❌ 沒有找到配置文件');
    console.log('💡 請先執行: node scripts/generate-yahoo-us-configs.js');
    process.exit(1);
  }
  ...
  const resultPath = path.join(__dirname, `../logs/yahoo-us-financials-batch-${timestamp}.json`);
